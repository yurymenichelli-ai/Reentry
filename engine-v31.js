...
export function spendingPace(data, reference = new Date()) {
  ...
  const rawSpendable = Math.max(0,beforeComfort-comfortMargin);
  ...
  const hasFundedDebtPlan=Boolean(plan.remainingDebt&&automaticDebtChoice?.feasible&&Number(automaticDebtChoice?.rate)>0);
  const maximumCycleLiving=hasFundedDebtPlan?Math.max(0,cycleAvailable-plan.minimumPayments-savingSetAside):automaticRawSpendable;
  ...
  const automaticLivingBudget=hasFundedDebtPlan?Math.max(0,(automaticDebtChoice?.livingMoney||0)-savingSetAside):automaticRawSpendable;
  const chosenPlanHasBudget=Boolean(plan.remainingDebt&&debtChoice?.feasible&&Number(debtChoice?.rate)>0);
  const recommendedSpendable=Math.min(automaticRawSpendable,automaticLivingBudget),spendable=hasChoice?Math.min(rawSpendable,chosenPlanHasBudget?Math.max(0,debtChoice?.livingMoney||0):rawSpendable):recommendedSpendable;
  ...
  return { available:true, mainIncome, payday, estimatedPayday:!mainIncome.due, daysRemaining, liquidNow, upcomingIncome, upcomingExpenses:allUpcomingExpenses, futureIncome, futureExpenses, debtSetAside, savingSetAside, comfortMargin, rawSpendable, spendable, maximumSpendable, maximumDaily, hasSpendingChoice:hasChoice, plannedSpend, projectedRemainder, daily, weekly };
}
