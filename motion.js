...
export function revealPage(root) {
  ...
  const sections = [...root.querySelectorAll('main > .reveal, main > .plan-summary, main > .goal-section, main > .advance-control, .scenario, .analysis-grid > .reveal, .list-row.transaction, .saving-section, .quota-row')];
  const enter = (element, delay = 0) => {
    element.animate([{opacity:0,transform:'translateY(12px)'},{opacity:1,transform:'translateY(0)'}],
      {duration:320,delay,easing:'cubic-bezier(.22,1,.36,1)',fill:'backwards'});
    element.querySelectorAll('.ring-value').forEach(ring => {
      ring.animate([{strokeDashoffset:100},{strokeDashoffset:0}],{duration:600,easing:'cubic-bezier(.22,1,.36,1)'});
    });
    element.querySelectorAll('.chart-track i,.progress-track i,.route-progress i').forEach(bar => {
      bar.animate([{transform:'scaleX(0)',transformOrigin:'left'},{transform:'scaleX(1)',transformOrigin:'left'}],{duration:450,easing:'cubic-bezier(.22,1,.36,1)'});
    });
  };
  // No content is hidden while waiting for the observer, including without JS.
  observer = new IntersectionObserver(entries => {
    entries.filter(entry=>entry.isIntersecting).forEach((entry,index)=>{
      if (!reducedMotion()) enter(entry.target,Math.min(index*25,75));
