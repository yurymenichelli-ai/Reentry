...
function syncSpendingEntry(transaction){
  ...
  if(transaction.type==='expense') state.spendingPeriods[0].entries.push({transactionId:transaction.id,label:transaction.label,amount:transaction.amount,category:transaction.category||'Altro'});
}
...
function dashboard(){
  const p=calculatePlan(state), capital=totalCapital(state), balances=capitalBalances(state), pace=spendingPace(state), progress=Math.min(100, Math.round((1-p.debtTotal/8200)*100));
  ...
  return shell(`${header(todayLabel.charAt(0).toUpperCase()+todayLabel.slice(1),`Buongiorno, ${state.name}.`,`<div class="header-actions"><button class="button secondary" data-capital>Imposta capitale</button><button class="button secondary desktop-only" data-add>${icons.plus} Aggiungi</button></div>`)}
    ...
    <section class="capital-hero reveal"><div><p class="eyebrow">Capitale disponibile</p><div class="capital-value">${state.capitalConfigured?money(capital):'Da impostare'}</div>${state.capitalConfigured?`<button class="text-button" data-toggle-detail>Mostra dettaglio</button><div class="capital-detail" hidden><span>Contanti <b>${money(balances.cash)}</b></span><span>Conto <b>${money(balances.account)}</b></span></div>`:`<button class="text-button" data-capital>Inserisci il saldo reale</button>`}</div>
    ...
    ${pace.available?`<section class="pace-card reveal"><div class="pace-heading"><div><span class="eyebrow">Ritmo fino al prossimo accredito</span><h2>${pace.mainIncome.name||'Entrata principale'} · ${pace.payday.toLocaleDateString('it-IT',{day:'numeric',month:'long'})}${pace.estimatedPayday?' (stima)':''}</h2><p>Puoi mantenere questo ritmo per ${pace.daysRemaining} giorni considerando tutti i movimenti già registrati.</p></div><span class="pace-status"><i></i>Margine incluso</span></div><div class="pace-values"><div><span>Al giorno</span><strong>${money(pace.daily)}</strong></div><div><span>In 7 giorni</span><strong>${money(pace.weekly)}</strong></div></div><details class="pace-details"><summary>Come abbiamo calcolato questa cifra?</summary><div><p><span>Capitale dopo i movimenti</span><b>${money(pace.liquidNow)}</b></p>${pace.upcomingIncome?`<p><span>Altre entrate prima dell’accredito</span><b>+ ${money(pace.upcomingIncome)}</b></p>`:''}<p><span>Spese programmate prima dell’accredito</span><b>− ${money(pace.upcomingExpenses)}</b></p><p><span>Accantonamento piano di rientro</span><b>− ${money(pace.debtSetAside)}</b></p><p><span>Margine per stare comodo</span><b>− ${money(pace.comfortMargin)}</b></p><p class="pace-total"><span>Disponibile per le spese quotidiane</span><b>${money(pace.spendable)}</b></p></div></details></section>`:`<section class="pace-card pace-empty"><div><span class="eyebrow">Ritmo di spesa</span><h2>${pace.reason==='capital'?'Imposta il capitale reale':'Aggiungi un’entrata ricorrente'}</h2><p>Ci servono capitale ed entrata principale per calcolare quanto puoi spendere fino al prossimo accredito.</p></div><button class="button secondary" ${pace.reason==='capital'?'data-capital':'data-view="transactions"'}>Completa i dati</button></section>`}
...
function onboarding(){
 const steps=[
  ...
  {title:'Quanto entra ogni mese?',copy:'Indica l’entrata principale e quando arriva. Servirà anche per calcolare il ritmo di spesa fino al prossimo accredito.',fields:`<label>Entrate mensili<div class="money-input"><b>€</b><input name="income" required type="number" min="1" value="${state.incomes[0]?.amount||''}" placeholder="Es. 1.800"></div></label><label>Giorno di accredito <span>opzionale</span><input name="incomeDue" type="number" min="1" max="28" value="${state.incomes[0]?.due||''}" placeholder="Es. 27"></label>`},
...
function modal(defaultType='expense',transaction={}){const type=transaction.type||defaultType;const category=transaction.category||'Altro';return `<div class="modal-backdrop"><form class="modal movement-modal ${type==='income'?'is-income':''}" id="movement-form"><button class="close" type="button" data-close aria-label="Chiudi">×</button><span class="kicker">${transaction.id?'Modifica movimento':'Movimento occasionale'}</span><h2>${transaction.id?'Correggi la registrazione':'Aggiorna il capitale'}</h2><input type="hidden" name="id" value="${transaction.id||''}"><div class="type-toggle"><label><input type="radio" name="type" value="expense" ${type==='expense'?'checked':''}><span>Spesa</span></label><label><input type="radio" name="type" value="income" ${type==='income'?'checked':''}><span>Entrata</span></label></div><label>Descrizione<input name="label" required value="${transaction.label||''}" placeholder="Es. Farmacia"></label><label>Categoria<select name="category">${['Casa','Alimentari','Trasporti','Tempo libero','Salute','Altro'].map(x=>`<option ${x===category?'selected':''}>${x}</option>`).join('')}</select></label><label class="scheduled-match">Voce programmata <span>opzionale</span><select name="recurringId"><option value="">Nessuna</option>${state.expenses.map(x=>`<option value="${x.id}" ${String(x.id)===String(transaction.recurringId)?'selected':''}>${x.name}</option>`).join('')}</select><small>Evita di conteggiare due volte una spesa già pagata.</small></label><label>Importo<div class="money-input"><b>€</b><input name="amount" required type="number" min="0.01" step="0.01" value="${transaction.amount||''}" placeholder="0,00"></div></label><fieldset class="channel-choice"><legend>Dove si muovono i soldi?</legend><label><input type="radio" name="channel" value="account" ${transaction.channel==='cash'?'':'checked'}><span><b data-channel-account>${type==='income'?'Bonifico o accredito':'Bancomat o carta'}</b><small>Aggiorna il conto</small></span></label><label><input type="radio" name="channel" value="cash" ${transaction.channel==='cash'?'checked':''}><span><b>Contanti</b><small>Aggiorna la cassa</small></span></label></fieldset><p class="form-error" hidden>Inserisci descrizione e importo.</p><button class="button wide" type="submit">${transaction.id?'Salva modifiche':'Salva movimento'}</button></form></div>`; }
...
document.addEventListener('submit',e=>{
 ...
 if(e.target.id==='movement-form'){
  ...
  const id=String(fd.get('id')||'');const existing=state.transactions.find(x=>String(x.id)===id);const transaction={id:existing?.id||Date.now(),label,amount,type:String(fd.get('type')),channel:fd.get('channel')==='cash'?'cash':'account',category:String(fd.get('category')||'Altro'),recurringId:fd.get('type')==='expense'&&fd.get('recurringId')?String(fd.get('recurringId')):null,recordedAt:existing?.recordedAt||new Date().toISOString().slice(0,10),date:existing?.date||'Oggi'};
  if(existing)state.transactions=state.transactions.map(x=>String(x.id)===id?transaction:x);else state.transactions.unshift(transaction);
  syncSpendingEntry(transaction);save();e.target.closest('.modal-backdrop').remove();render();toast(existing?'Movimento aggiornato. Saldi ricalcolati.':'Movimento salvato. Saldi aggiornati.');return;
 }
 ...
 if(e.target.id==='onboarding-form'){
  ...
  if(onboardingStep===2) state.incomes=[{id:1,name:'Stipendio o entrata principale',amount:Number(fd.get('income')),frequency:'monthly',due:Number(fd.get('incomeDue'))||null}];
