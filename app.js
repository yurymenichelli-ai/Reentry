...
function dashboard(){
  const p=calculatePlan(state), capital=totalCapital(state), balances=capitalBalances(state), progress=Math.min(100, Math.round((1-p.debtTotal/8200)*100));
...
document.addEventListener('submit',e=>{
 ...
 if(e.target.id==='movement-form'){
  ...
  const id=String(fd.get('id')||'');const existing=state.transactions.find(x=>String(x.id)===id);const transaction={id:existing?.id||Date.now(),label,amount,type:String(fd.get('type')),channel:fd.get('channel')==='cash'?'cash':'account',category:String(fd.get('category')||'Altro'),date:existing?.date||'Oggi'};
...
document.addEventListener('change',e=>{if(e.target.matches('#movement-form input[name="type"]')){const label=e.target.form.querySelector('[data-channel-account]');if(label)label.textContent=e.target.value==='income'?'Bonifico o accredito':'Bancomat o carta';}});
